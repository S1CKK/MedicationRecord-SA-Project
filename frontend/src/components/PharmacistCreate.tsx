...
function PharmacistCreate() {
  const classes = useStyles();
  const [selectedDate, setSelectedDate] = useState<Date | null>(new Date());
  const [pharmacist, setPharmacist] = useState<Partial<PharmacistInterface>>({});
  ...
  const handleInputChange = (
    ...
    const id = event.target.id as keyof typeof pharmacist;
    const { value } = event.target;
    setPharmacist({ ...pharmacist, [id]: value });
  };
    ...
    <Container className={classes.container} maxWidth="md">
      ...
      <Paper className={classes.paper}>
        ...
        <Grid container spacing={3} className={classes.root}>
          ...
          <Grid item xs={12}>
            <Button component={RouterLink} to="/pharmacists" variant="contained">
              กลับ
            </Button>
            <Button
